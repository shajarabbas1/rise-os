...
export async function middleware(request: NextRequest) {
  ...
  if (isPublicPath && accessToken) {
    return NextResponse.redirect(new URL(PAGES_ROUTES.careerDashboard, request.url));
  }
  ...
  try {
    ...
    const validateToken = await axiosInstance.get(ENDPOINT.validateToken, {
      headers: {
    ...
    if (!validateToken.status) {
      const response = NextResponse.redirect(new URL(PAGES_ROUTES.login, request.url));
